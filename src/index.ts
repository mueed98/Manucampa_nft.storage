...
const NFT_STORAGE_TOKEN: string = process.env.API_KEY
	? process.env.API_KEY
	: "";
...
async function uploadImages() {
	...
	for (let i = 0; i < FILE_COUNT; i++) {
		...
		imageArray.push(
			new File([readFiledata], `${i}.png`, { type: "image/png" })
		);
	}
...
async function uploadVideos() {
	...
	for (let i = 0; i < FILE_COUNT; i++) {
		...
		videoArray.push(
			new File([readFiledata], `${i}.mp4`, { type: "video/mp4" })
		);
	}
...
async function updateMetaDataFiles(imageCid: string, videoCid: string) {
	...
	for (let i = 0; i < FILE_COUNT; i++) {
		...
		MetaDataArray.push(
			new File([buf], `${i}.json`, { type: "application/json" })
		);
	}
